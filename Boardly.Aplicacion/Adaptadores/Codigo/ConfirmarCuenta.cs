...
public class ConfirmarCuenta: IConfirmarCuenta<Resultado>
{
    ...
    public ConfirmarCuenta(ICodigoRepositorio codigoRepositorio, IUsuarioRepositorio usuarioRepositorio)
    {
     ...
     public async Task<Resultado> ConfirmarCuentaAsync(Guid usuarioId, string codigo, CancellationToken cancellationToken)
    {
        ...
        var codigoEntidad = await _codigoRepositorio.BuscarCodigoAsync(codigo, cancellationToken);
        ...
        var esValido = await _codigoRepositorio.ElCodigoEsValidoAsync(codigo, cancellationToken);
        if (!esValido)
        {
            return Resultado.Fallo(Error.Fallo("400", "El código ha expirado o no es válido"));
        }

        await _codigoRepositorio.MarcarCodigoComoUsado(codigoEntidad.Valor!, cancellationToken);

        usuario.CuentaConfirmada = true;
        await _usuarioRepositorio.ActualizarAsync(usuario, cancellationToken);
